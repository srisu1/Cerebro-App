...
class AvatarCustomizationScreen extends ConsumerStatefulWidget {
  final bool isSetup;
  const AvatarCustomizationScreen({super.key, this.isSetup = false});
  @override
...
class _AvatarCustomizationScreenState
  ...
  final _femaleClothesColors = ['green','red','blue'];
  final _maleClothesColors = ['black','blue','red','green'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: LayoutBuilder(builder: (context, box) {
        ...
        // right section — fit everything on screen
        ...
        // dynamic tab gap — shrink gaps so tabs fit
  ...
  // right section with avatar preview and controls
  ...
  // uses OverflowBox + Transform.scale on native image sizes
  Widget _buildAvatar(double pvSize) {
    final containerScale = pvSize / 1000.0;
    final layers = <_Layer>[];

    // base → eyes → nose → mouth
    ...
    layers.add(_Layer('assets/avatar/$_gender/clothes/$_clothesStyle-$_clothesColor.png', clothesPos.x, clothesPos.y, clothesPos.scale));
  ...
  List<Widget> _buildDots() {
    if (_currentCategory == 'hair') {
      return _hairColors.map((c) => _dot(c, _hairColor == c, () => setState(() => _hairColor = c))).toList();
    }
  ...
  Widget _dot(String key, bool sel, VoidCallback onTap) {
    ...
    switch (_currentCategory) {
      ...
      case 'clothes':
        ...
        return _grid(styles.map((s) => _optBox(
          ...
          child: Image.asset('assets/avatar/$_gender/clothes/$s-$_clothesColor.png',
            fit: BoxFit.contain, errorBuilder: (_, __, ___) => Text(s.split('-').first, style: GoogleFonts.nunito(fontSize: 10, fontWeight: FontWeight.w700))),
        )).toList());
      case 'facialHair':
        return _grid([
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _grid([
        ...
        for (final g in _glassesOptions)
          _optBox(selected: _glassesStyle == g, onTap: () => setState(() => _glassesStyle = g),
      ...
      _grid([
        ...
        for (final h in _headwearOptions)
          _optBox(selected: _headwearStyle == h, onTap: () => setState(() { _headwearStyle = h; _headwearColor ??= 'red'; }),
      ...
      _grid([
        ...
        for (final n in _neckwearOptions)
          _optBox(selected: _neckwearStyle == n, onTap: () => setState(() { _neckwearStyle = n; _neckwearColor ??= 'red'; }),
      ...
      if (_gender == 'female')
        _grid([
          ...
          for (final e in _extrasOptions)
            _optBox(selected: _extrasStyle == e, onTap: () => setState(() { _extrasStyle = e; _extrasColor ??= 'red'; }),
  ...
  Widget _optBox({required bool selected, required VoidCallback onTap, required Widget child}) {
    return GestureDetector(
      onTap: onTap,
      child: AnimatedScale(
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: CerebroTheme.outline, width: selected ? 3 : 2.5),
                    boxShadow: selected
...
// bounce button for game-like feel
